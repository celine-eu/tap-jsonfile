# AGENTS.md - AI Agent Development Guide for tap-jsonfile
...
This tap follows the Singer specification and uses the Meltano Singer SDK to extract data from JsonFile.

### Key Components

1. **Tap Class** (`tap_jsonfile/tap.py`): Main entry point, defines streams and configuration
1. **Client** (`tap_jsonfile/client.py`): Handles API communication and authentication
1. **Streams** (`tap_jsonfile/streams.py`): Define data streams and their schemas
...
1. Define stream class in `tap_jsonfile/streams.py`
...
```python
class MyNewStream(JsonFileStream):
    name = "my_new_stream"
...
```bash
tap-jsonfile --config config.json --discover
tap-jsonfile --config config.json --catalog catalog.json
```
...
1. Update `config_jsonschema` in `tap_jsonfile/tap.py`
...
Before filing, search existing issues to avoid duplicates. Include the SDK version (`uv run tap-jsonfile --version`), Python version, and a minimal reproduction case when reporting bugs.
